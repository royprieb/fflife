...
from blog.forms import newAccountForm
from blog.forms import accountEditForm
from blog.forms import loginForm
from blog.forms import journalEditForm
from blog.forms import carForm
from blog.forms import carEditForm
from blog.forms import modForm
from blog.forms import albumForm
...
def journalVisit(request, name):
    ...
    if len(cars) ==  0:
        ...
        navSearchForm = SearchForm()
        cars = []
        return render(request, 'journalvisit.html',{
                      'journal': j,
                      'cars': cars,
                      'navSearchForm':navSearchForm,
                      'next': request.path,
...
def postVisit(request, name, carId, postId):
...
def carVisit(request, name, carId):
    c = Car.objects.get(pk=carId)
    tags = c.tags.all()
    j = Journal.objects.get(name=name)
    cars = Car.objects.filter(journal=j)
...
@login_required
def stopFollow(request, name, carId):
    u = request.user
    c = Car.objects.get(pk=carId)
    j = Journal.objects.get(name=name)
...
@login_required
def message(request, name, ownerId):
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            # create an event
            next = 'http://fastfreshlife.com/'
            next += j.owner.username
            next += '/message/'
...
def accountEdit(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            journal = Journal.objects.get(owner=u)
...
def modAll(request, name, carId):
    if request.user.username != name:
        ...
        return render(request, 'modAll.html',{
                      'journal':j,
                      'cars':cars,
                      'car':c,
                      'tags':tags,
...
@login_required
def modView(request,name, carId, modId):
    return HttpResponseRedirect('nothing here')
...
@login_required
def albumView(request, name, carId, albumId):
    if request.user.username != name:
        return HttpResponseRedirect('/%s' % name)
    else:
        #searchform
...
@login_required
def albumAdd(request, name, carId, albumId):
    if request.user.username != name:
        return HttpResponseRedirect('/%s' % name)
    else:
        #searchform
...
@login_required
def albumEdit(request, name, carId, albumId):
    if request.user.username != name:
        return HttpResponseRedirect('/%s' % name)
    else:
        #searchform
...
def photoDelete(request, name, photoId):
